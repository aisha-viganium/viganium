...
export default async function Blogs({ locale, t }: { locale: string; t: (key: string) => string }) {
    ...
    const blogs = {
        ...
        "en": [

            {
                ...
                "image": "/assets/images/blog.png",
                "description": "It is simply dummy text (meaning that the purpose is form and not content) and is used in"
            },
            {
                "id": 2,
                "name": "Custom Software Development",
    ...
    const blogsData = isArabic ? blogs.ar : blogs.en;
    return(
        <>
            ...
            <div>
                <AnimatedSection className={`${isArabic ? "border-r-[13px] md:border-r-[36px] border-r-[#F9C751] pr-2 md:pr-4" : "border-l-[13px] md:border-l-[20px] border-l-[#F9C751] pl-2 md:pl-4"}  min-h-[33px] flex items-center`}>
                    <p className={`${isArabic ? "text-right" : "text-left"}  max-w-[796px] 
                        ...
                        font-bold 
                        text-secondry`}>
                        {t("BlogsPage.BlogsSection.title")}
                    </p>

                </AnimatedSection>
                <main className="mt-0 md:mt-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-6 ">
                        {blogsData.filter(Boolean).map((blog, index) => {
                            if (!blog) return null;
